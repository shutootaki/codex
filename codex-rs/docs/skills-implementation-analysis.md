...
ユーザー入力からLLMリクエストまでの関数呼び出しチェーンを詳細に解説する。
...
│                                                     ▼                   │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │      user_input_or_turn() (codex.rs:1734)                        │  │
│  │              ↓                                                    │  │
│  │      sess.spawn_task(ctx, items, RegularTask)  (1784行目)        │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                     │                   │
│                                                     ▼                   │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │      spawn_task() (tasks/mod.rs:106)                             │  │
│  │              ↓                                                    │  │
│  │      tokio::spawn(task.run(...))  (126行目)                      │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                     │                   │
│                                                     ▼                   │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │      RegularTask::run() (tasks/regular.rs:24)                    │  │
│  │              ↓                                                    │  │
│  │      run_task() (codex.rs:2205)                                  │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                     │                   │
│                                                     ▼                   │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │      run_task() のターンループ (codex.rs:2262)                   │  │
│  │      loop {                                                       │  │
│  │          run_turn() ────────────────────────────────┐             │  │
...
│                                                        ▼                │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │      try_run_turn() (codex.rs:2504)                              │  │
│  │              ↓                                                    │  │
│  │      ModelClient::stream(prompt) ← LLMリクエスト送信              │  │
│  │              ↓                                                    │  │
│  │      SSEイベントループ処理                                        │  │
│  └──────────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────────────┘
...
| 関数 | ファイル:行 | 役割 |
|------|-------------|------|
| `submission_loop()` | codex.rs:1578 | イベントループ。`Op` を受信して適切なハンドラに振り分け |
| `user_input_or_turn()` | codex.rs:1734 | ユーザー入力を処理し、`spawn_task` を呼び出す |
| `spawn_task()` | tasks/mod.rs:106 | タスクを `tokio::spawn` で非同期実行 |
| `RegularTask::run()` | tasks/regular.rs:24 | `SessionTask` trait の実装。`run_task` を呼び出す |
| `run_task()` | codex.rs:2205 | **複数ターンのループ**。Skills注入、自動圧縮を担当 |
| `run_turn()` | codex.rs:2365 | **リトライループ**。ネットワークエラー時の再試行 |
| `try_run_turn()` | codex.rs:2504 | **LLMストリーム処理**。イベントを受信してツール実行 |

### 1.11.3 3層のループ構造

Codexは3層のループ構造を持ち、それぞれ異なる責務を担う：

```
┌─ submission_loop ─────────────────────────────────────────────────┐
│  ユーザー入力待ちループ（アプリケーション全体のライフサイクル）    │
│                                                                    │
│  ┌─ run_task ───────────────────────────────────────────────────┐ │
│  │  複数ターンループ（1つの作業の完了まで）                      │ │
│  │                                                               │ │
│  │  ┌─ run_turn ──────────────────────────────────────────────┐ │ │
│  │  │  リトライループ（1回のLLM呼び出しの成功まで）            │ │ │
│  │  │                                                          │ │ │
│  │  │  • ネットワークエラー → 再試行                           │ │ │
│  │  │  • 成功 → 結果を返す                                     │ │ │
│  │  │  • 致命的エラー → エラーを返す                           │ │ │
│  │  └──────────────────────────────────────────────────────────┘ │ │
│  │                                                               │ │
│  │  • needs_follow_up = true → 次のターンへ（ツール結果を送信） │ │
│  │  • needs_follow_up = false → ループ終了（作業完了）          │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                    │
│  • Op::Shutdown → ループ終了                                      │
│  • 次のユーザー入力を待機                                          │
...
**作業が完了したかどうかは `needs_follow_up` フラグで判断される**：
...
```rust
// codex.rs:2311 - run_task() 内
if !needs_follow_up {
    // LLMがツールを呼ばずにテキストを返した
    // → ユーザーへの最終回答 → タスク完了
    sess.notifier().notify(&UserNotification::AgentTurnComplete { ... });
    break;  // ループ終了
}
// needs_follow_up = true の場合 → 次のターンへ continue
```

**設計のポイント**:
- **LLM駆動の完了判断**: LLM自身がツールを呼ばなくなった時点で作業完了
- **無限ループ防止**: トークン上限での自動圧縮（`run_auto_compact`）
- **レジリエンス**: ネットワークエラー時の自動リトライ
- **キャンセル可能**: 全レイヤーで `CancellationToken` を伝播

### 1.11.5 実際の動作例

```
ユーザー: 「ファイルを読んで内容を要約して」

┌─ ターン1 ─────────────────────────────────────────────────────────┐
│ LLM出力: FunctionCall { name: "read_file", path: "README.md" }    │
...
└───────────────────────────────────────────────────────────────────┘
        ↓ continue (次のターンへ)
┌─ ターン2 ─────────────────────────────────────────────────────────┐
│ LLM入力: [ユーザーメッセージ, ツール結果(ファイル内容)]            │
│ LLM出力: Message { "このファイルは〇〇について説明しています..." } │
